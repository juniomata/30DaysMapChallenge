...
jakarta_parks$osm_polygons <- st_make_valid(jakarta_parks$osm_polygons)
jakarta_parks$osm_multipolygons <- st_make_valid(jakarta_parks$osm_multipolygons)
...
jakarta_water_sf <- st_union(jakarta_water$osm_polygons, jakarta_water$osm_multipolygons)
jakarta_parks_sf <- st_union(jakarta_parks$osm_polygons, jakarta_parks$osm_multipolygons)
...
# Define colors
background_fill <- "#FFF"   
water_fill <- "#B3DDEE" #"#28A7F5"
park_fill <-  "#CFEFD7"  #"#56C7AD"
road_color <- "#767C83" #"#656A70"

# Plot map
jakarta_map <- ggplot() +
  geom_sf(data = jakarta_water_sf, fill = water_fill, color = NA) +
  geom_sf(data = jakarta_parks_sf, fill = park_fill, color = NA) +
...
# Save as PNG
ggsave("jakarta_map.png", plot = jakarta_map, width = 10, height = 10, dpi = 300, units = "in")
...
crs_projected <- 3857
jakarta_water_sf <- jakarta_water_sf %>% st_transform(crs_projected)
jakarta_parks_sf <- jakarta_parks_sf %>% st_transform(crs_projected)
...
water_masked <- st_intersection(jakarta_water_sf, circle_mask)
parks_masked <- st_intersection(jakarta_parks_sf, circle_mask)
...
jakarta_circular_map <- ggplot() +
  ...
  geom_sf(data = water_masked, fill = water_fill, color = NA) +
  geom_sf(data = parks_masked, fill = park_fill, color = NA) +
...
ggsave("jakarta_circular_map.png", plot = jakarta_circular_map, width = 10, height = 10, dpi = 300, units = "in")
...
jakarta_map_image <- image_read("jakarta_circular_map.png")

# Set up the output PNG file using ragg
agg_png("jakarta_circular_map_annotated.png", width = 4000, height = 4000, res = 300)

# Start a new grid plot
grid.newpage()
grid.rect(gp = gpar(fill = "#FFFFFF", col = NA)) # Background fill
...
grid.text(
  ...
  just = "center", gp = gpar(fontfamily = "lato", fontsize = 250, col = "#333333", fontface = "plain")
)
...
grid.text(
  ...
  just = "center", gp = gpar(fontfamily = "lato", fontsize = 150, col = "#333333", fontface = "plain")
)
...
grid.text(
  ...
  just = "right", gp = gpar(fontfamily = "sans", fontsize = 41, col = "#AAA", lineheight = 0.3)
)
...
annotated_map <- image_read("jakarta_circular_map_annotated.png")
